...
// Load once per process and cache in module scope.
let corpus: KoreaFood[] | null = null;

function loadCorpus(): KoreaFood[] {
  if (corpus) return corpus;
  const filePath = path.join(process.cwd(), 'lib', 'data', 'korea-kfct.json');
  const raw = fs.readFileSync(filePath, 'utf-8');
  corpus = JSON.parse(raw) as KoreaFood[];
  return corpus;
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error('[Korea KFCT] Failed to load corpus:', e);
    return NextResponse.json({ error: 'Korea KFCT data failed to load', detail: String(e) }, { status: 502 });
  }
  ...
  const scored = data
    ...
    .filter((x): x is { f: KoreaFood; score: number } => x !== null)
