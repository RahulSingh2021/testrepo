...
const CANVAS_W = 794;
const CANVAS_H = 562;
const STORE_KEY = 'haccppro_lm_cert_studio_v1';
const GALLERY_KEY = 'haccppro_lm_cert_gallery_v1';
...
async function fetchTemplatesFromDB(): Promise<DesignTemplate[]> {
  try {
    const res = await fetch('/api/lm-cert-templates');
    if (!res.ok) return [];
...
async function fetchGalleryFromDB(): Promise<GalleryImage[]> {
  try {
    const res = await fetch('/api/lm-cert-gallery');
    if (!res.ok) return [];
    const data = await res.json();
...
// MAIN EXPORT
// ═══════════════════════════════════════════════════════════════════════════════
export default function LMCertificateStudio() {
