...
const generateTasksForRange = (equipmentList: ConnectedEquipment[], startDate: Date, endDate: Date, facilityChecklists?: ChecklistTemplate[]): PMTask[] => {
    const tasks: PMTask[] = [];
    ...
    equipmentList.forEach(eq => {
        ...
        if (!eq.pmStartDate) return;

        const val = eq.pmFrequencyValue || 1;
        const unit = eq.pmFrequencyUnit || 'Days';
        const preferredDayIdx = eq.pmDay ? DAY_MAP[eq.pmDay] : -1;

        // 1. Determine Initial Anchor Date
        let anchorDate = new Date(eq.pmStartDate);
        ...
        while (safetyCounter < 5000) {
            ...
            if (cycleStart >= start && cycleStart <= end) {
                ...
                let status: PMTask['status'] = 'scheduled';
                ...
                const linkedChecklist = facilityChecklists?.find(c => c.title === eq.pmChecklist);
                const pmPageQuestions = linkedChecklist?.pages?.[0]?.sections?.reduce((sum, s) => sum + (s.questions?.length || 0) + ((s as any).subSections || []).reduce((ss: number, sub: any) => ss + (sub.questions?.length || 0), 0), 0) || 0;
                const checkpointCount = pmPageQuestions > 0 ? pmPageQuestions : 5;
                ...
                tasks.push({
                    ...
                    frequency: `Every ${val} ${unit}${eq.pmDay ? ` on ${eq.pmDay}` : ''}`,
                    assignedDay: eq.pmDay || 'Dynamic',
                    status: status,
                    totalCheckpoints: checkpointCount,
                    responsibility: 'Staff',
                    isCarryOver: isCarryOver,
                    daysOverdue: daysOverdue,
                    checklistName: eq.pmChecklist || undefined,
                });
            }
...
const generateMockTasks = (): PMTask[] => {
    const today = new Date();
...
const TaskCard: React.FC<{ task: PMTask, onAttend: () => void, onVerify: () => void, onHistory?: () => void, onReschedule?: () => void, viewDate?: string }> = ({ task, onAttend, onVerify, onHistory, onReschedule, viewDate }) => {
      ...
      <div className="pl-4 md:pl-0 flex flex-col lg:flex-row items-stretch border-b border-slate-100">
        <div className="p-4 md:p-6 lg:flex-1 flex items-start gap-4 shrink-0">
          ...
          <div className="flex flex-col items-center gap-1.5 shrink-0">
            <div className={`w-11 h-11 md:w-12 md:h-12 rounded-2xl flex items-center justify-center shadow-inner ${isVerified ? 'bg-emerald-50 text-emerald-600' : isOverdue || isCarriedOver ? 'bg-rose-50 text-rose-600' : 'bg-orange-50 text-orange-600'}`}>
              <Wrench size={20} />
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm flex flex-col gap-3">
                      <div className="flex items-center gap-3 border-b border-slate-50 pb-2">
                          ...
                          <div>
                              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-0.5">Maintained By</p>
...
const HistoryModal: React.FC<{ task: PMTask, onClose: () => void }> = ({ task, onClose }) => {
    ...
    const historyData = useMemo(() => {
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            history.push({
                ...
                maintenanceDate: new Date(date.getTime() + 1000 * 60 * 60 * 15).toISOString(), // same day 3pm
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col border border-slate-200 animate-in zoom-in-95 h-[80vh]">
                ...
                <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-start bg-white">
                    <div className="flex flex-col gap-1">
                        <div className="text-xs font-bold text-slate-500 uppercase tracking-wider">Equipment Maintenance History</div>
                ...
                <div className="flex-1 overflow-hidden flex flex-col">
                    <div className="px-6 py-4 flex justify-between items-center bg-white border-b border-slate-100">
                         <div className="flex items-center gap-2 text-indigo-600">
                             <History size={18} />
                             <h3 className="font-bold text-sm">Maintenance Schedule</h3>
                         </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead className="bg-slate-50 text-xs font-bold text-slate-400 uppercase tracking-wider sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 border-b border-slate-200">Maintenance</th>
                            ...
                            <tbody className="text-sm text-slate-600 divide-y divide-slate-100">
                                {historyData.map((row, idx) => (
                                    <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="font-bold text-slate-800">{row.technician}</div>
                                            <div className="text-[10px] text-slate-400 mt-0.5">{new Date(row.maintenanceDate).toLocaleString('en-GB', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true })}</div>
                                        </td>
                                        <td className="px-6 py-4">
...
const PM_METHODS: Record<string, { method: string; tools: string[]; chemicals: string[] }> = {
  'Standard PM Inspection': { method: 'Visual inspection, check operating parameters, lubricate moving parts, tighten connections', tools: ['Multimeter', 'Torque wrench', 'Lubricant applicator', 'Inspection mirror'], chemicals: ['Food-grade lubricant', 'Contact cleaner'] },
  'Full Service Overhaul': { method: 'Disassemble key components, inspect wear parts, replace filters/gaskets, recalibrate sensors, test run', tools: ['Socket set', 'Bearing puller', 'Dial indicator', 'Torque wrench', 'PPE gloves'], chemicals: ['Industrial degreaser', 'Food-grade lubricant', 'Descaler'] },
  'Quick Operational Check': { method: 'Verify temperature readings, check door seals, test safety cutoffs, inspect power connections', tools: ['Thermometer', 'Inspection checklist', 'Voltage tester'], chemicals: [] },
  'Compressor Service': { method: 'Check refrigerant levels, clean condenser coils, inspect electrical connections, verify thermostat accuracy', tools: ['Manifold gauge set', 'Coil brush', 'Fin comb', 'Leak detector'], chemicals: ['Coil cleaner', 'Refrigerant'] },
};

const CHECKLIST_ITEMS = [
  'Pre-maintenance safety lockout/tagout completed',
  'Visual inspection of all components completed',
  'Operating temperatures within specification',
  'All moving parts lubricated per schedule',
  'Electrical connections checked and tightened',
  'Filters/gaskets inspected and replaced if needed',
  'Safety cutoffs and alarms tested',
  'Calibration verified against reference standards',
  'Equipment reassembled and test run completed',
...
const generatePlanRecords = (eq: ConnectedEquipment): PMPlanRecord[] => {
  const records: PMPlanRecord[] = [];
  const today = new Date();
  today.setHours(0,0,0,0);
  const val = eq.pmFrequencyValue || 1;
  const unit = eq.pmFrequencyUnit || 'Days';
  ...
  while (safety < 5000 && cycleDate <= sevenDaysAhead) {
    if (cycleDate >= thirtyDaysAgo) {
      ...
      let status: PMPlanRecord['status'] = 'upcoming';
      if (cycleDate < today) {
      ...
      records.push({
        ...
        comments: (status === 'completed' || status === 'verified') ? (hash % 3 === 0 ? 'All components inspected as per SOP. No issues found.' : hash % 5 === 0 ? 'Minor belt wear detected, replacement scheduled for next cycle.' : 'Routine maintenance completed successfully.') : undefined,
...
const PreventiveMaintenanceModule: React.FC<PreventiveMaintenanceModuleProps> = ({ equipmentList = [], facilityChecklists = [] }) => {
  ...
  const getLinkedChecklist = (task: PMTask): ChecklistTemplate | undefined => {
    if (!task.checklistName) return undefined;
    return facilityChecklists.find(c => c.title === task.checklistName);
  };

  const buildDefaultTemplate = (task: PMTask): ChecklistTemplate => {
    ...
    const defaultQuestions: QuestionNode[] = [
      makeQ(1, 'Equipment powered down and lockout/tagout procedure followed'),
      makeQ(2, 'All moving parts inspected for wear, damage, or misalignment'),
      makeQ(3, 'Lubrication applied to bearings, chains, and pivot points per schedule'),
      makeQ(4, 'Electrical connections inspected, tightened, and free of corrosion'),
      makeQ(5, 'Filters, belts, and gaskets inspected and replaced if necessary'),
      makeQ(6, 'Safety interlocks, guards, and emergency stops tested and functional'),
      makeQ(7, 'Operating parameters verified within manufacturer specifications'),
      makeQ(8, 'Equipment reassembled, test run completed, and performance verified'),
    ];
    const sections: SectionNode[] = [
      { id: `ds-${task.id}-1`, title: 'PM Inspection Points', isApplicable: true, risk: 'Med', category: 'Maintenance', questions: defaultQuestions },
    ];
    const pages: PageNode[] = [
      { id: `dp-${task.id}-1`, title: `${task.equipmentName} — PM Checklist`, sections },
    ];
    return {
      id: `auto-pm-${task.id}`,
      title: `${task.equipmentName} — PM Checklist`,
      pages,
      createdDate: new Date().toISOString(),
    };
  };

  const handleOpenAttendModal = (task: PMTask) => {
  ...
  const handleOpenVerifyModal = (task: PMTask) => { setActiveTask(task); setVerifyComments(""); setVerifySignature(""); setIsVerifyModalOpen(true); };
  const handleOpenHistory = (task: PMTask) => { setActiveTask(task); setIsHistoryModalOpen(true); };
  const handleOpenReschedule = (task: PMTask) => { setActiveTask(task); setNewScheduleDate(task.scheduledDate); setRescheduleReason(""); setIsRescheduleModalOpen(true); };
  ...
  const generateFilledPDF = (record: PMPlanRecord, equipmentName: string, equipmentId: string, checklist: string) => {
    const doc = new jsPDF();
    ...
    doc.setFontSize(10);
    doc.text('PM CHECKLIST REPORT', pageWidth - margin - 50, yPos);
    yPos += 10;
    ...
    doc.text('PREVENTIVE MAINTENANCE COMPLETION REPORT', pageWidth / 2, yPos, { align: 'center' });
    yPos += 10;
    doc.setTextColor(0, 0, 0);

    // General Information and Maintenance Details
    doc.setFontSize(11);
    ...
    doc.setFontSize(11);
    doc.text('MAINTENANCE DETAILS', pageWidth / 2 + 10, yPos - 13);
    doc.setFontSize(9);
    ...
    doc.text(`Maintained By: ${record.completedBy || '--'}`, pageWidth / 2 + 10, yPos + 2);
    yPos += 8;
    ...
    if (hasBeforePhotos || hasAfterPhotos || hasGeneralPhotos) {
      ...
      if (hasBeforePhotos) {
        ...
        doc.text('BEFORE MAINTENANCE', margin, yPos);
        doc.setTextColor(0, 0, 0);
        yPos += 8;
      ...
      if (hasAfterPhotos) {
        ...
        doc.text('AFTER MAINTENANCE', margin, yPos);
        doc.setTextColor(0, 0, 0);
        yPos += 8;
  ...
  const planEquipmentData = useMemo(() => {
    ...
    return list.filter(eq => eq.status === 'Active').map(eq => {
      ...
      const methodInfo = PM_METHODS[eq.pmChecklist] || PM_METHODS['Standard PM Inspection'];
    ...
    <div className="flex flex-col gap-6 pb-20 animate-in fade-in duration-500">
      ...
      <div className="bg-white p-3 sm:p-5 rounded-2xl sm:rounded-[2.5rem] border border-slate-200 shadow-xl flex flex-col gap-4 sm:gap-6 relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 sm:gap-6 z-10">
          <div className="p-2.5 sm:p-4 bg-orange-50 text-orange-600 rounded-2xl sm:rounded-3xl shadow-inner border border-orange-100 flex-shrink-0">
            <ClipboardCheck size={24} className="sm:block hidden" />
            <ClipboardCheck size={18} className="sm:hidden" />
          </div>
          <div>
            <h2 className="text-lg sm:text-xl lg:text-2xl font-black text-slate-900 tracking-tighter uppercase leading-none">PM Registry</h2>
            <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 mt-1 sm:mt-2 uppercase tracking-[0.15em] flex items-center gap-1 sm:gap-2">
              <ShieldCheck size={10} className="sm:block hidden text-orange-500" /> <span className="hidden sm:inline">Preventive Maintenance Performance Dashboard</span><span className="sm:hidden">PM Dashboard</span>
            </p>
          </div>
        </div>
      ...
      <div className="flex bg-slate-100 p-1 sm:p-1.5 rounded-xl sm:rounded-2xl border border-slate-200 self-start overflow-x-auto">
        <button 
          ...
          className={`px-3 sm:px-5 lg:px-8 py-2 sm:py-3 rounded-lg sm:rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-1 sm:gap-2 flex-shrink-0 ${activeSubTab === 'schedule' ? 'bg-white shadow-md text-orange-600 ring-1 ring-black/5' : 'text-slate-400 hover:text-slate-600'}`}
        >
          <CalendarDays size={12} className="sm:block hidden" /> <span className="hidden sm:inline">PM Schedule</span><span className="sm:hidden">Schedule</span>
        </button>
        <button 
          onClick={() => setActiveSubTab('plan')}
          className={`px-3 sm:px-5 lg:px-8 py-2 sm:py-3 rounded-lg sm:rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-1 sm:gap-2 flex-shrink-0 ${activeSubTab === 'plan' ? 'bg-white shadow-md text-orange-600 ring-1 ring-black/5' : 'text-slate-400 hover:text-slate-600'}`}
        >
          <ListCheck size={12} className="sm:block hidden" /> <span className="hidden sm:inline">PM Plan</span><span className="sm:hidden">Plan</span>
        </button>
      </div>
            ...
            {filteredTasks.length > 0 ? filteredTasks.map(task => (
                ...
                <div className="py-24 flex flex-col items-center justify-center text-center bg-white rounded-[2.5rem] border-2 border-dashed border-slate-200">
                    ...
                    <p className="text-slate-400 text-xs mt-3 font-medium uppercase tracking-widest max-w-xs">
                        There are no PM assignments matching your filter for this period.
                    </p>
                </div>
            )}
        </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between px-1">
          <h3 className="text-lg font-black text-slate-800 uppercase tracking-tight flex items-center gap-3">
            <ListCheck size={18} className="text-orange-600" />
            Equipment PM Plan
          </h3>
          <span className="bg-slate-100 text-slate-500 px-3 py-1 rounded-full text-[10px] font-black uppercase border border-slate-200">
        ...
        <div className="grid grid-cols-1 gap-4">
          {filteredPlanEquipment.length > 0 ? filteredPlanEquipment.map(eq => {
                ...
                <button onClick={() => togglePlanExpand(eq.id)} className="w-full text-left p-5 lg:p-6">
                  <div className="flex flex-col lg:flex-row lg:items-start gap-4">
                    <div className="flex items-start gap-4 flex-1 min-w-0">
                      <div className="w-12 h-12 bg-gradient-to-br from-orange-500 to-amber-600 rounded-2xl flex items-center justify-center shadow-lg shadow-orange-500/20 shrink-0">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex flex-wrap gap-2 mt-3">
                          ...
                          <span className="inline-flex items-center gap-1 text-[9px] font-bold uppercase tracking-wider text-slate-500 bg-slate-50 border border-slate-200 px-2 py-1 rounded-lg">
                            <Repeat size={10} /> Every {eq.pmFrequencyValue} {eq.pmFrequencyUnit}
                          </span>
                  ...
                  <div className="border-t border-slate-100 bg-slate-50/50 p-4 lg:p-6 space-y-3">
                    <div className="flex items-center justify-between mb-2">
                      <h5 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                        <History size={12} /> Date-wise PM Records
                      </h5>
                          ...
                          <div key={rec.date} className={`rounded-xl border p-3 ${statusConfig.bg} transition-all hover:shadow-sm`}>
                            ...
                            {(rec.status === 'completed' || rec.status === 'verified') && (
                              <div className="space-y-2 mt-2">
                                ...
                                <button
                                  onClick={() => generateFilledPDF(rec, eq.name, eq.idNumber, eq.pmChecklist)}
                            ...
                            {rec.status === 'missed' && (
                              <p className="text-[10px] text-rose-500 font-medium mt-1 italic">Maintenance was not performed on this scheduled date.</p>
                            )}
                            {rec.status === 'upcoming' && (
            ...
            <div className="py-24 flex flex-col items-center justify-center text-center bg-white rounded-[2.5rem] border-2 border-dashed border-slate-200">
              ...
              <p className="text-slate-400 text-xs mt-3 font-medium uppercase tracking-widest max-w-xs">
                Add active equipment in Facility Management to see PM plans here.
              </p>
            </div>
          )}
        </div>
      ...
      {showPdfModal && pdfDataUrl && (
        <div className="fixed inset-0 z-[200] bg-white flex flex-col animate-in fade-in duration-200">
          <div className="px-8 py-6 bg-slate-900 text-white flex justify-between items-center shrink-0 shadow-lg">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-orange-600 rounded-2xl shadow-lg shadow-orange-600/20"><ClipboardCheck size={28} strokeWidth={2} /></div>
              <div>
                <h3 className="text-2xl font-black uppercase tracking-tight">Filled PM Checklist</h3>
                <p className="text-slate-400 text-sm mt-1">View and download your complete maintenance report</p>
              </div>
            </div>
          ...
          <div className="px-8 py-5 border-t border-slate-100 bg-white flex justify-between items-center shrink-0 shadow-lg">
            ...
            <a
              href={pdfDataUrl}
              download={`pm-report-${new Date().toISOString().split('T')[0]}.pdf`}
              className="px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest text-white bg-orange-600 hover:bg-orange-700 transition-all flex items-center justify-center gap-2"
            >
